...
document.querySelector(".content__link--open");
const linksOpen = document.querySelectorAll(".content__link--open");
...
linksOpen.forEach((e) => {
  e.addEventListener("click", () => {
    console.log(e);
    if (openPage) {
      openPage.classList.add("open");
    }
  });
});
btnClose$1.addEventListener("click", () => {
  if (openPage.classList.contains("open")) {
    openPage.classList.add("closing");
    setTimeout(() => {
      openPage.classList.remove("open");
      openPage.classList.remove("closing");
    }, 500);
  }
});
...
const circleArr = document.querySelectorAll(".anim__circles");
const isTouchDevice = window.matchMedia("(hover: none)").matches;
circleArr.forEach((element) => {
  const animBlock = element.closest(".anim");
  if (!animBlock) return;
  if (isTouchDevice) {
    element.addEventListener("click", () => {
      document.querySelectorAll(".anim.svg-active").forEach((activeBlock) => {
        if (activeBlock !== animBlock) {
          activeBlock.classList.remove("svg-active");
        }
      });
      animBlock.classList.toggle("svg-active");
    });
  } else {
    element.addEventListener("mouseenter", () => {
      animBlock.classList.add("svg-active");
    });
  }
});
    ...
    new Swiper(".swiper", {
      ...
      */
      ...
      pagination: {
        ...
        renderFraction: (currentClass, totalClass) => `<span class="${currentClass}"></span>
...
const btnSwich = document.querySelector(".form__btn--swich");
const callBackBlock = document.querySelector(".call-back");
const btnClose = document.querySelector(".form__btn--close ");
btnSwich.addEventListener("click", (e) => {
  e.preventDefault();
  callBackBlock.classList.add("show-block");
});
btnClose.addEventListener("click", () => {
  setTimeout(() => {
    callBackBlock.classList.remove("show-block");
  }, 1e3);
});
function menuInit$1() {
  document.addEventListener("click", function(e) {
...
document.querySelectorAll("[data-fls-fullpage-section]:not(:first-child)").forEach((section) => {
  new MutationObserver((mutations, obs) => {
    if (section.classList.contains("--watcher-view")) {
      const phone = section.querySelector(".phone");
      if (phone) {
        setTimeout(() => {
          phone.classList.add("phone--loop");
        }, 2800);
      }
      obs.disconnect();
    }
